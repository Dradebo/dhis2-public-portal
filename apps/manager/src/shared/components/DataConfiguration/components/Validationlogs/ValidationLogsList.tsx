...
export function ValidationLogsList({ logs, isLoading, error }: ValidationLogsListProps) {
    ...
    if (error) {
        return (
            <NoticeBox error title={i18n.t('Failed to load validation logs')}>
                {error.message || i18n.t('An error occurred while loading the validation logs')}
            </NoticeBox>
        );
    }
    ...
    if (logs.length === 0) {
        return (
            <div
                className="bg-gray-50 text-gray-700 font-mono text-sm p-4 rounded border border-gray-300 min-h-96"
            >
                <div className="flex items-center gap-2 text-gray-700">
                    <span>⚡</span>
                    <span>{i18n.t('Validation console ready. Logs will appear here when validation starts.')}</span>
                </div>
            </div>
        );
    }
    ...
    return (
        ...
        >
            ...
            <div className="space-y-1">
                {logs.map((log, index) => (
                    <div key={log.id} className="flex items-start gap-3 py-1 hover:bg-gray-100 px-2 -mx-2 rounded">
                        {/* Timestamp */}
                        ...
                        {/* Log Level */}
                        <span className={`text-xs font-semibold whitespace-nowrap ${getLevelColor(log.level)}`}>
